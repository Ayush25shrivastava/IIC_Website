...
vec3 shade(vec2 uv, vec2 p, float t) {
  float y = uv.y + sin(uv.x * (3.0 + u_intensity * 9.0) + t * 0.8) * 0.08 + (fbm(p * 2.0 + t * 0.1) - 0.5) * u_intensity * 0.6;
  return palette(y);
}
...
const UNIFORMS = {
  colors: [
    [0.09411764705882353, 0.27058823529411763, 0.3411764705882353],
    [0, 0.4980392156862745, 0.6784313725490196],
    [0.3411764705882353, 0.6980392156862745, 0.796078431372549],
...
export default function AdisyonShader({ className = "" }) {
  ...
  useEffect(() => {
    ...
    const updatePointerTarget = () => {
      ...
      if (!inside) {
        targetPresence = 0; requestRender(); return
      }
    ...
    function render(now) {
      ...
      if (timeAnimated || Math.abs(targetX - mouseX) > 0.001 || Math.abs(targetY - mouseY) > 0.001 || Math.abs(targetPresence - cursorPresence) > 0.001) requestRender()
      else lastNow = null
    }
    requestRender()
    return () => {
