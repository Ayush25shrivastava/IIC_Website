...
// Import images from assets/current sponsors
const images = import.meta.glob('../assets/current sponsors/*', { eager: true, query: '?url', import: 'default' });
            ...
            <div className="relative w-full overflow-hidden bg-[#dcd9d2]/30 py-4 border-y border-[#b8a18a]/30 backdrop-blur-sm">
                ...
                <div className="flex w-max animate-scroll-reverse hover:pause">
                    ...
                    {sponsorLogos.map((src, idx) => (
                        <div key={`current-sponsor-1-${idx}`} className="mx-8 w-32 h-16 flex items-center justify-center transition-all duration-300 transform hover:scale-110">
                            <img src={src} alt={`Sponsor ${idx}`} className="max-w-full max-h-full object-contain filter drop-shadow-md" />
                        </div>
                    ))}
                    {/* Second set for infinite loop */}
                    {sponsorLogos.map((src, idx) => (
                        <div key={`current-sponsor-2-${idx}`} className="mx-8 w-32 h-16 flex items-center justify-center transition-all duration-300 transform hover:scale-110">
                            <img src={src} alt={`Sponsor ${idx}`} className="max-w-full max-h-full object-contain filter drop-shadow-md" />
                        </div>
                    ))}
                </div>
