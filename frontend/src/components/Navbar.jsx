...
const Navbar = () => {
    ...
    return (
        <>
            ...
            {/* Mobile Menu Overlay */}
            <div className={`fixed inset-0 bg-[#0E0E0E] z-40 flex flex-col items-center justify-center transition-transform duration-300 md:hidden ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                {/* Close Button */}
                <button
                            ...
                            <>
                                <div className="flex items-center gap-3 mb-4">
                                    <img
                                        ...
                                        alt={user.name}
                                        referrerPolicy="no-referrer"
                                        className="w-12 h-12 rounded-full border border-[#C5A25F] object-cover"
                                        onError={(e) => { e.target.src = 'https://via.placeholder.com/32' }}
                                    />
                                    <span className="text-[#C5A25F] font-merriweather text-lg">
                                        {user.name}
                                    </span>
                                </div>
                                <button
                                    onClick={() => { handleLogout(); toggleMobileMenu(); }}
                                    className="px-8 py-3 border border-[#C5A25F] text-[#C5A25F] font-merriweather text-lg font-bold uppercase hover:bg-[#C5A25F] hover:text-[#0E0E0E] transition-all duration-300"
                                >
                                    Logout
                                </button>
                            </>
